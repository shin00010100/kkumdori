...
const ProductList = ({ onProductClick }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
        let apiUrl = "/api/goods/goodslist";
        if (categoryNo) {
            apiUrl = `/api/goods/goodslist/${categoryNo}`;
        }

        console.log(`Fetching products from: ${apiUrl}?query=${searchQuery}&page=${currentPage}&size=${itemsPerPage}`);

        try {
            const response = await fetch(
                `${apiUrl}?query=${searchQuery}&page=${currentPage}&size=${itemsPerPage}`,
                { method: "GET" }
            );
            ...
            if (data && data.products) {
                ...
                setTotalItems(data.totalItems || 0); // 총 상품 개수 설정
    ...
    fetchProducts();
}, [categoryNo, searchQuery, currentPage]);
